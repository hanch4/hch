...
{
    if(netif == 0)
        netif_set_link_down(r91h_wifidata.netif_sta);
    else
        netif_set_link_down(r91h_wifidata.netif_ap);
}
...
{
    if(netif == 0)
        netif_set_link_up(r91h_wifidata.netif_sta);
    else
        netif_set_link_up(r91h_wifidata.netif_ap);
}
...
void r91h_wifiif_input(struct netif *netif, u8_t *data, u32_t len, int idx)
{
  ...
  p = r91h_low_level_input(netif, data, len, idx);
  if (p == NULL) {
    return;
  }
