...
protected:
    ...
    */
    ...
    virtual bool EP0Continue(void);
    virtual bool EP0Status(void);
    virtual void EP0Complete(void);
    void fillControlTransfer(uint8_t requestType, uint8_t request, uint16_t value, uint16_t index, int len, SETUP_PACKET *packet);
    ...
    virtual uint8_t get_dma_channel_status(uint8_t channel);
    virtual void set_dma_channel_status(uint8_t channel, uint8_t status);
    virtual void dmaCallback(uint8_t channel);
#endif
    virtual void deviceDisconnectSendMsg();
    virtual void deviceDisconnect(int hub, int port, USBHostHub * hub_parent, volatile uint32_t addr);
...
private:
    ...
    BULK_TRANSFER in_transfer;
    BULK_TRANSFER out_transfer;
#ifdef CFG_USB_DMA
    USB_DMA_CHANNEL in_channel;
    USB_DMA_CHANNEL out_channel;
#endif /* CFG_USB_DMA */
#endif /* TARGET_UNO_91H */
