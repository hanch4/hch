...
int do_wake_source(cmd_tbl_t *cmd, int argc, char *argv[])
{
    ...
    enable = atoi(argv[2]);
    if(0 == src) { // uart
        user_sleep_wakesrc_set(WAKESRC_UART, enable, 0);
    } else if(1 == src) { // gpio
        int gp_num = 0;
        if(enable) {
            if(argc < 4) {
        ...
        user_sleep_wakesrc_set(WAKESRC_GPIO, (unsigned int)enable, (unsigned int)gp_num);
    } else if(2 == src) { // timer, valid only in powerdown mode
        unsigned int count_10ms = 0UL;
        if(enable) {
            if(argc < 4) {
                RDA_AT_PRINT("WrongArgCnt:%d\r\n", argc);
                return 0;
            }
            count_10ms = atoi(argv[3]);
        }
        user_sleep_wakesrc_set(WAKESRC_TIMER, (unsigned int)enable, count_10ms);
    }
    RESP_OK();
    return 0;
}

int do_boot_addr(cmd_tbl_t *cmd, int argc, char *argv[])
{
    unsigned int addr = 0x00UL;
    if(argc < 2) {
        RESP_ERROR(ERROR_ARG);
        return 0;
    }
    addr = simple_strtoul(argv[1], NULL, 16);
    user_sleep_bootaddr_set(addr);
    RESP_OK();
    return 0;
}
#endif
